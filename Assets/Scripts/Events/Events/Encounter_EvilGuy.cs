...
/*
public class Encounter_EvilGuy : Encounter
{
    // Static
    public override int DefName => 3;
    ...
    public Encounter_EvilGuy(Game game) : base(game) { }
    public override Encounter GetEventInstance => new Encounter_EvilGuy(Game);
    ...
    private EncounterStep GetStandoffStep(string text)
    {
        // Dialogue Options
        ...
        return new EncounterStep(text, dialogueOptions, itemOptions, allowDefaultItemInteractions: false);
    }
    private EncounterStep Fight()
    {
        // Success chance
        ...
        {
            WinFight();
            return new EncounterStep("You jump on the guy and manage to knock him unconcious before he can hurt you. You take his stuff and leave.");
        }
        else if(fightSuccess && gotInjured)
        {
            GetInjured();
            WinFight();
            return new EncounterStep("You jump on the guy and a dirty fight ensues. You manage to knock him about but take a punch in the process. You take his stuff and leave.");
        }
        else
        {
    ...
    private EncounterStep PayRansom(Item ransomItem)
    {
        if (ransomItem != null) Game.DestroyOwnedItem(ransomItem);
        return new EncounterStep("You give the guy your " + ransomItem.Name + ". He thanks and wishes you a nice day.");
    }
    ...
    private EncounterStep ThrowItem(Item throwItem)
    {
        ...
        {
            WinFight();
            return new EncounterStep("Bullseye! The " + throwItem.Name + " knocked the guy straight to the floor. You take his stuff and leave.", null, null);
        }
        else
        {
            string text = "The " + throwItem.Name + " missed the guy and gets destroyed upon hitting the floor.";
            EncounterStep startStep = GetStandoffStep(text);
            return startStep;
        }
    }

...
public static class HelperFunctions
{
    ...
    public static T GetWeightedRandomElement<T>(Dictionary<T, float> weightDictionary)
    {
        ...
        if (Game.DEBUG_RANDOM_CHOICES)
        {
            ...
            probabilites += "\n------------------------------";
            foreach (KeyValuePair<T, float> kvp in normalizedProbabilites.Where(x => x.Value > 0).OrderByDescending(x => x.Value))
            {
                string label = kvp.Key.ToString();
                probabilites += "\n" + (kvp.Key.Equals(chosenValue) ? "* " : "") + label + ": " + kvp.Value.ToString("0.0") + "%";
            }
            probabilites += "\n------------------------------";
